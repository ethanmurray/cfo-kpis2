...
export async function handleAsk(body: AskRequest): Promise<AskResponse> {
  const { question, filters, conversationHistory, clientId } = body

  // Look up client config for prompt generation
  const config = clientId ? getClientConfig(clientId) : undefined
  ...
  try {
    return await handleWithE2B(question, filters, historyContext, clientId, config)
  } catch (e2bError) {
    ...
    return await handleDirectAnalysis(question, filters, historyContext, clientId, config)
  }
}
...
async function handleWithE2B(
  ...
  const systemPrompt = config ? getCodeGenerationSystemPrompt(config) : getCodeGenerationSystemPrompt()
...
${stdout ? `Partial stdout output:\n${stdout}` : 'No stdout output was produced.'}`
  ...
  const narrativeSystemPrompt = config ? getNarrativeSystemPrompt(config) : getNarrativeSystemPrompt()
  let narrative = ''
...
async function handleDirectAnalysis(
  ...
  const dataJson = getDataJson(clientId)
  const systemPrompt = config ? getDirectAnalysisSystemPrompt(config) : getDirectAnalysisSystemPrompt()
